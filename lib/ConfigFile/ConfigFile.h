...
template <uint8_t K> boolean ConfigFile<K>::begin(const char *configFileName) {
  ...
  if (!_file.open(configFileName,O_RDONLY)) {
#ifdef CONFIGFILE_DEBUG
...
/*
 * Cleans up our ConfigFile File object.
 */
template <uint8_t K> void ConfigFile<K>::end() {
  if (_file) {
    _file.close();
  }
  _atEnd = true;
 ...
 */
template <uint8_t K> boolean ConfigFile<K>::readNextSetting() {
  int bint;
 ...
 */
template <uint8_t K> boolean ConfigFile<K>::nameIs(const char *name) {
 ...
 */
template <uint8_t K> const char *ConfigFile<K>::getName() {
...
/*
IPAddress ConfigFile::getIPAddress(){
